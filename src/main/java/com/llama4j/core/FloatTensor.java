...
public abstract class FloatTensor {
    ...
    static short readShort(@NotNull MemorySegment memorySegment, long offset) {
        // The MemorySegment.get* methods should be used instead.
        return UNSAFE.getShort(memorySegment.address() + offset);
    }

    static byte readByte(@NotNull MemorySegment memorySegment, long offset) {
        // The MemorySegment.get* methods should be used instead.
    ...
    void copyTo(int thisOffset, @NotNull FloatTensor that, int thatOffset, int size) {
